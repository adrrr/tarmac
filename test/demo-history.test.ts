...
test('the invented month has a shape: quiet weekends, ordinary weeks, and a day that went long', async () => {
  ...
  const ends = days.filter((d) => weekday(d.date) === 0 || weekday(d.date) === 6).map(spend);
  const weeks = days.filter((d) => weekday(d.date) !== 0 && weekday(d.date) !== 6).map(spend);
  assert.ok(ends.length >= 8 && weeks.length >= 20, `the month holds ${ends.length} weekend days and ${weeks.length} weekdays`);
