...
// 120-character model laid the document out at 814px until these two rules went on it.
//
// Context, cost and uptime are NOT in this list, and not because of where they come from: a cost
// is copied out of that same payload and guarded even less than the model (`1e999` is legal JSON
// and reaches the cell as `$Infinity`, where a percentage of that shape is refused). What bounds
// them is what they are PRINTED through — a percentage clamped to 0..100 and floored, a duration,
// and a `toFixed(2)` that goes exponential long before it goes long. None can be a token wider
// than a phone, and a rule guarding that is prose that lies.
