...
- **A day of what `serve` already read, on `GET /api/history`.** The dashboard read the whole
  ...
  account's rate limits and the session count beside them. In memory and nowhere else, by
  design: a fleet journal on disk would outlive the process that wrote it and is the one file
  this tool promised never to write, so the record starts when the serve starts and `since`
  says so out loud. **No name enters the ring, for any kind of session** — a background
