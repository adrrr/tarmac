...
- **A named pipe at backup.json no longer freezes `install` and `uninstall`.** The fix above
  covered settings.json; the backup beside it was still read with no question about its kind, and
  it is the other file both commands read before they print anything — so one FIFO there and the
  run froze exactly as before, with no output and nothing to press. The kind is asked before the
  open now, and a backup.json that is not a regular file stops the run by naming the file. A
  missing or corrupt backup still reads as "no usable install", as it always did: that answer
  lets a fresh install write over what is there, and a file we never opened must not wear it
  (#193).
...
- **`health.covered`, `health.unfilable` and `health.drift` are counted over the sessions a
  ...
  equal and nothing moves. And when the printed denominator is smaller than the fleet, both
  coverage lines say who they left out — `(2 agent(s) draw no frame)` — so the banner and the
  rows under it cannot disagree on how many are missing.
