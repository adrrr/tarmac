...
- **`tarmac serve --demo`, so the first run is not an empty screen.** One session and no record
  ...
  replay and the curves are all worth looking at from the first second. It reads nothing and
  writes nothing: no `claude` spawned, no snapshot directory opened, no temp file swept, and no
  journal whatever `--history-days` says. A `demo data` badge sits beside the title on every
