...
its marker in place so that command never points at a missing file. And if the wrapper itself
is already gone — deleted by hand, with `backup.json` left behind, which uninstall still works
through — nothing says where that directory is: uninstall opens none, and the plan prints
